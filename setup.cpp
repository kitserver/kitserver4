...
// Returns the game version id for compressed EXE.
// We're using section headers to identify the file here.
int CheckForCompressedExe(FILE* f)
{
	IMAGE_SECTION_HEADER secHeader;
	for (int i=0; i<NUM_SIG_HEADERS; i++) {
		ZeroMemory(&secHeader, sizeof(IMAGE_SECTION_HEADER));
		if (SeekSectionHeader(f, (char*)SIG_HEADER[i].Name)) {
			fread(&secHeader, sizeof(IMAGE_SECTION_HEADER), 1, f);
			if (memcmp(&secHeader, &SIG_HEADER[i], sizeof(IMAGE_SECTION_HEADER))==0) {
				return i + NUM_GUIDS;
			}
		}
	}
...
int GetGameVersion(char* filename)
{
	...
	// check for compressed exes
	int gv = CheckForCompressedExe(f);
	fclose(f);
	return gv;
}
...
void MyMessageBox(char* fmt, DWORD value)
{
	...
	MessageBox(hWnd, buf, "KitServer 4 DEBUG MyMessage", 0);
	*/
}
...
%s.", fileName);
		...
		sprintf(buf, "\
======== SUCCESS! =========\n\
Setup has installed KitServer 4 for\n\
%s.", fileName);
	...
	else
	{
		...
		sprintf(buf, "\
======== ERROR! =========\n\
Setup failed to install KitServer 4 for\n\
%s.\n\
\n\
	...
	if (f != NULL)
	{
		...
		if (dataOffset != 0) {
			...
			if (newEntryPoint == 0)
			{
				...
				sprintf(buf, "\
======== INFORMATION! =========\n\
KitServer 4 is not installed for\n\
%s.", fileName);
		...
		sprintf(buf, "\
======== SUCCESS! =========\n\
Setup has removed KitServer 4 from\n\
%s.", fileName);
	...
	else
	{
		...
		sprintf(buf, "\
======== ERROR! =========\n\
Setup failed to remove KitServer 4 from\n\
%s.\n\
\n\
(No modifications made.)\n\
Verify that the executable is not\n\
